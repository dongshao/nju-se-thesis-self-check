...
- 步骤 9 切分时正文字符数 < 5000（明显抽取失败）→ 跳过步骤 9 并在报告里写明原因
...
[3/9] 已加载 engineering-50.md（49 条）
[4/9] 红线扫描：命中 X 条
[5/9] 全量审查完成：✅ A / ⚠️ B / ❌ C
[6/9] 图表规范性：识别 N 张图 / 命中 M 类图 / 警告 K 条
[7/9] 报告写入 {report_path}
[8/9] 摘要：预估评议结论 {结论}
[9/9] 文风+错别字检查：N 块切分 / 错别字 X 条 / 文风 Y 条
完成。
```
...
import fitz, os
PDF = "/path/to/paper.pdf"
FIGS_DIR = os.path.join(os.path.dirname(PDF), "figs")
os.makedirs(FIGS_DIR, exist_ok=True)
...
### 8.2 AI 文风痕迹（humanizer-zh 24 类模式）
...
3. 一句话："详细列表已追加到报告 `{report_path}` 第八节"
